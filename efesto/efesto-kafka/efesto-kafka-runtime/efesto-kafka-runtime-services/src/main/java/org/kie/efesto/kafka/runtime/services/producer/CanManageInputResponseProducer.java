...
public class CanManageInputResponseProducer {

    private static final Logger logger = LoggerFactory.getLogger(CanManageInputResponseProducer.class);

    private static final AtomicLong COUNTER = new AtomicLong();


    public static void runProducer(boolean toPublish, long messageId) {
        logger.info("runProducer");
        final Producer<Long, JsonNode> producer = createProducer();
        runProducer(producer, toPublish, messageId);
    }

    public static void runProducer(final Producer<Long, JsonNode> producer, boolean toPublish, long messageId) {
        ...
        try {
            ...
            final ProducerRecord<Long, JsonNode> record =
                    new ProducerRecord<>(RUNTIMESERVICE_CANMANAGEINPUTRESPONSE_TOPIC, COUNTER.incrementAndGet(), jsonNode);
    ...
    static JsonNode getJsonNode(boolean toPublish, long messageId) {
        EfestoKafkaRuntimeCanManageInputResponseMessage responseMessage = new EfestoKafkaRuntimeCanManageInputResponseMessage(toPublish, messageId);
    ...
    private static Producer<Long, JsonNode> createProducer() {
        ...
        props.put(ProducerConfig.BOOTSTRAP_SERVERS_CONFIG,
                BOOTSTRAP_SERVERS);
        props.put(ProducerConfig.CLIENT_ID_CONFIG, CanManageInputResponseProducer.class.getSimpleName());
